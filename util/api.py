...
class Api:
    ...
    def get_avg_deal_value_for_last_year(self):
        all_deals = self.get_deals()
        won_deals_last_year = list(filter(self.deal_is_won_last_year, all_deals))
        return round(
            functools.reduce(lambda a, b: a + b["value"], won_deals_last_year, 0) / len(won_deals_last_year))

    def updated_company_statuses(self):
        updated_companies = self.get_companies()
        all_deals = self.get_deals()
        for company in updated_companies:
            company_deals = list(filter(lambda x: self.get_deals_won_by_company(x, company["_id"]), all_deals))
            company_deals_last_year = list(
                filter(lambda x: self.deal_is_won_in_the_last_year_by_company(x, company["_id"]), all_deals))
    ...
    def get_value_per_company(self):
        all_deals = self.get_deals()
        won_deals_last_year = list(filter(self.deal_is_won_last_year, all_deals))
    ...
    def deal_is_won_last_year(self, deal):
    ...
    def get_deals_won_by_company(self, deal, company):
        if not deal["closeddate"]:
            return False
        return deal["dealstatus"]["key"] == "agreement" and deal["company"] == company


    def deal_is_won_in_the_last_year_by_company(self, deal, company_id):
        if not deal["closeddate"]:
            return False
        one_year_ago = datetime.now().replace(tzinfo=timezone(offset=timedelta())) - relativedelta(years=1)
    ...
    def get_companies(self):
        if self.companies is None:
            self.companies = self.get_api_data(
